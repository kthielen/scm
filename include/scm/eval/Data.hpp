...
public:
	...
	typedef std::set<unsigned int> UInts;

	ValArray(const Values& vs = Values());
	ValArray(unsigned int n);
	...
	void insert(unsigned int n, Value* v);
	void insert(unsigned int n, const Values& v);

	void remove(unsigned int n);
	void remove(const UInts& ns);

	Value* cut(unsigned int from, unsigned int to) const;
	void del(unsigned int from, unsigned int to);
