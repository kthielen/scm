...
std::string& String::value()                         { return this->str; }
...
// an array data type
ValArray::ValArray(const Values& vs) : data(vs) { }
ValArray::ValArray(unsigned int n) { this->data.resize(n); }
Value* ValArray::get(unsigned int n) { return n >= this->data.size() ? 0 : this->data[n]; }

void ValArray::set(unsigned int n, Value* v) {
	if (n >= this->data.size()) {
...
void ValArray::insert(unsigned int n, Value* v) {
	if (n > this->data.size()) {
		this->data.resize(n);
	}
	this->data.insert(this->data.begin() + n, v);
}

void ValArray::insert(unsigned int n, const Values& v) {
	if (n > this->data.size()) {
		this->data.resize(n);
	}
	this->data.insert(this->data.begin() + n, v.begin(), v.end());
}

void ValArray::remove(unsigned int n) {
	if (n < this->data.size()) {
		this->data.erase(this->data.begin() + n);
	}
}

void ValArray::remove(const UInts& ns) {
	for (UInts::const_reverse_iterator n = ns.rbegin(); n != ns.rend(); ++n) {
		remove(*n);
	}
}

template <typename T>
	inline T cap(const T& x, const T& ul) {
		return (x < ul) ? x : ul;
	}

Value* ValArray::cut(unsigned int from, unsigned int to) const {
	if (this->data.size() == 0) {
		return this->allocator()->allocate<ValArray>();
	} else {
		if (to < from) {
			std::swap(to, from);
		}
		from = cap<unsigned int>(from, this->data.size());
		to   = cap<unsigned int>(to,   this->data.size());
	
		return this->allocator()->allocate<ValArray>(Values(this->data.begin() + from, this->data.begin() + to));
	}
}

void ValArray::del(unsigned int from, unsigned int to) {
	if (this->data.size() > 0) {
		from = cap<unsigned int>(from, this->data.size());
		to   = cap<unsigned int>(to,   this->data.size());
		this->data.erase(this->data.begin() + from, this->data.begin() + to);
	}
}

ValArray::Values& ValArray::value()             { return this->data; }
const ValArray::Values& ValArray::value() const { return this->data; }
int ValArray::size() const                      { return this->data.size(); }
void ValArray::resize(int n)                    { this->data.resize(n); }

void ValArray::format(std::ostream& out) const {
	out << "[";
	Values::const_iterator v = this->data.begin();
	if (v != this->data.end()) {
		PrintExpression(*v, out);
		++v;
		for (; v != this->data.end(); ++v) {
			out << ", ";
			PrintExpression(*v, out);
		}
	}
...
bool ValArray::operator==(const Value& rhs) const {
	...
	if (arhs == 0) {
		return false;
	} else if (arhs->data.size() != this->data.size()) {
		return false;
	}

	for (unsigned int i = 0; i < this->data.size(); ++i) {
		Value* lhs = this->data[i];
		Value* rhs = arhs->data[i];
...
void ValArray::references(AllocVisitor& vt) {
	for (Values::iterator v = this->data.begin(); v != this->data.end(); ++v) {
		vt.visit((AllocBase*&)*v);
	}
}
